...
#if MOBILE
    window = GPU_Init(0, 0, SDL_WINDOW_OPENGL|SDL_WINDOW_FULLSCREEN|SDL_WINDOW_ALLOW_HIGHDPI);

//    window = SDL_CreateWindow(NULL, 0, 0, NULL, NULL, );
    SDL_DisplayMode display_mode;
    SDL_GetCurrentDisplayMode(0, &display_mode);
//    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC);
//    SDL_GetRendererOutputSize(renderer, &render_screen_width, &render_screen_height);

    screen_width = window->w;
    screen_height = window->h;
    device_scale = 3;
    render_screen_width =  screen_width / device_scale;
    render_screen_height = screen_height / device_scale;
#else
    // (1 - 1/3)
...
static mrb_value create_texture(mrb_state *mrb, mrb_value self) {
    ...
    GPU_Image *new_texture;
    SDL_Surface *loaded_surface = GPU_LoadSurface(texture_name);

    if (loaded_surface == NULL) {
        printf("Unable to load image %s! SDL_image Error: %s\n", texture_name, IMG_GetError());
    } else {
        //Create texture from surface pixels
        new_texture = GPU_CopyImageFromSurface(loaded_surface);
        if (new_texture == NULL) {
...
static mrb_value draw_image(mrb_state *mrb, mrb_value self) {
    ...
    GPU_Image *texture = textures[texture_index];
...
//      SDL_SetTextureColorMod(texture, red, green, blue);
        GPU_SetRGBA(texture, red, green, blue, 255);
    }
...
//    SDL_RenderCopy(renderer, texture, NULL, &destination);
    GPU_Blit(texture, NULL, window, destination.x, destination.y);
    if (blend_factor > 0) {
        GPU_SetRGBA(texture, 255, 255, 255, 255);
    }
